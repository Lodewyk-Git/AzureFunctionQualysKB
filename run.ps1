...
function Normalize-KBFilterParameters {
    ...
    foreach ($pair in $pairs) {
        ...
        # Keep KB filters explicit; prevents unsupported options like truncation_limit.
        if ($key -ieq "details") {
...
function QualysKB {
    ...
    $parsedTimeInterval = 0
    if ([int]::TryParse($env:timeInterval, [ref]$parsedTimeInterval) -and $parsedTimeInterval -gt 0) {
    ...
    # Normalize filters and drop unsupported KB parameters.
    ...
    Try {
        Invoke-RestMethod -Headers $hdrs -Uri "$base/session/" -Method Post -Body $body -ContentType "application/x-www-form-urlencoded" -SessionVariable sess
    }
    catch {
    ...
    Write-Host ("UTC current Time : {0}" -f ([datetime]::UtcNow))
    $requestUri = "$base/knowledge_base/vuln/?action=list&published_after=$($startDate)$filterparameters"
    Write-Host "Constructed URL : $requestUri"

    $response = Try {
        Invoke-RestMethod -Headers $hdrs -Uri $requestUri -WebSession $sess
    }
    catch {
        if ($_.Exception.Response) {
    ...
    if($null -ne $response.KNOWLEDGE_BASE_VULN_LIST_OUTPUT.RESPONSE.VULN_LIST.VULN){
        ...
        try {
            ...
            $objs = @()
            0 .. ($response.KNOWLEDGE_BASE_VULN_LIST_OUTPUT.RESPONSE.VULN_LIST.VULN.Length - 1) | ForEach-Object {
                if($null -eq $response.KNOWLEDGE_BASE_VULN_LIST_OUTPUT.RESPONSE.VULN_LIST.VULN[$_].QID) {     # if the vuln ID is mull which will mean the entry is null, this occurs on the last entry of the response. Should only occur once.
                    ...
                    'THREAT_INTELLIGENCE' = $response.KNOWLEDGE_BASE_VULN_LIST_OUTPUT.RESPONSE.VULN_LIST.VULN[$_].THREAT_INTELLIGENCE.THREAT_INTEL | Select-Object id, "#cdata-section" | ConvertTo-Json -Depth 6
                    'DateValue' = $response.KNOWLEDGE_BASE_VULN_LIST_OUTPUT.RESPONSE.VULN_LIST.VULN[$_].LAST_SERVICE_MODIFICATION_DATETIME
                    }
        ...
        try {
            ...
            Write-Warning "Failed to logout session: $($_.Exception.Message)"
        }
        ...
        if ($objs.Length -ne 0){
            $jsonPayload = $objs | ConvertTo-Json -Depth 6
            ...
            if (($mbytes -le 30)){
                $responseCode = Post-LogAnalyticsData -customerId $customerId -sharedKey $sharedKey -body ([System.Text.Encoding]::UTF8.GetBytes($jsonPayload)) -logType $tableName -logAnalyticsUri $logAnalyticsUri
                ...
                Write-Error "ERROR: Log Analytics POST failed due to payload exceeding 30Mb: $mbytes"
                }
            }
            else {
            ...
            if ($_.Exception.Response) {
                Write-Host "Error while processing KB response, StatusCode:" $_.Exception.Response.StatusCode.value__
            }
            Write-Error -Message $_.Exception.Message
        }
    }
...
function Post-LogAnalyticsData($customerId, $sharedKey, $body, $logType, $logAnalyticsUri)
{
    ...
    $laUri = $logAnalyticsUri + $resource + "?api-version=2016-04-01"
    ...
    $response = Invoke-WebRequest -Body $body -Uri $laUri -Method $method -ContentType $contentType -Headers $headers -UseBasicParsing
